...
const ConditionsPage = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await healthcareCatalogApi.getConditions();
      setConditions(res?.data || res || []);
    } catch (err) {
      toast.error('Failed to load reference conditions');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      await healthcareCatalogApi.createCondition(form);
      toast.success('Condition created successfully!');
      setIsAddOpen(false);
      setForm({ name: '', type: 'GENERAL', description: '', isActive: true });
      loadData();
    } catch (err) {
      toast.error(err?.response?.data?.message || err?.message || 'Failed to save condition');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-black text-slate-900 flex items-center gap-2.5">
            Reference Range Conditions <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full uppercase font-bold">Catalogue</span>
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">Manage patient biological or collection states (e.g. Fasting, Pregnancy).</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-6">
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-1">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Total Conditions</span>
          <h3 className="text-2xl font-black text-slate-800">{conditions.length}</h3>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex gap-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search conditions by name or type..."
            value={search}
        ...
        <table className="w-full text-left border-collapse text-xs">
          ...
          <tbody className="divide-y divide-slate-55">
            {loading ? (
              ...
              <tr>
                <td colSpan="4" className="p-8 text-center text-slate-400 italic">No conditions match the search query.</td>
              </tr>
            ) : (
              filtered.map(c => (
                <tr key={c._id} className="hover:bg-slate-50/50">
                  <td className="p-4 font-black text-slate-800">{c.name}</td>
                  <td className="p-4 font-bold text-slate-700">{c.type}</td>
                  <td className="p-4 text-slate-400">{c.description || '—'}</td>
                  <td className="p-4 text-right">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${c.isSystemDefined ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-600'}`}>
                      {c.isSystemDefined ? 'System' : 'Custom'}
                    </span>
                  </td>
                </tr>
              ))
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-white rounded-3xl w-full max-w-md shadow-xl overflow-hidden border border-slate-100 flex flex-col">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-base font-black text-slate-900">Add Biological Condition</h3>
            ...
            <div className="p-6 space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-bold text-slate-500 uppercase">Condition Name *</label>
                <input
                  type="text"
                  required
