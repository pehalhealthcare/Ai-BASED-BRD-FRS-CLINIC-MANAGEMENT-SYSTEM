  ...
  return (
    ...
    >
      <div className="bg-white/95 border border-slate-200/80 shadow-2xl rounded-3xl p-8 max-w-sm w-full flex flex-col items-center text-center space-y-6 animate-scale-up relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-blue-500 to-purple-500 animate-shimmer" />

        {/* Branding (PEHAL Healthcare) */}
        <div className="flex flex-col items-center gap-1.5 mt-2">
          <div className="flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-black shadow-sm">P</span>
        ...
        <div className="relative w-20 h-20 flex items-center justify-center bg-blue-50/50 rounded-full border border-blue-100/50 shadow-inner">
          <div className="absolute inset-0 rounded-full border-2 border-blue-500 border-t-transparent animate-spin" />
          <PageIcon className="text-blue-600 animate-pulse float-slow" size={28} />
        </div>
        ...
        <div className="w-full text-left bg-slate-50 border border-slate-100 rounded-2xl p-4.5 space-y-2.5 max-h-[120px] overflow-y-auto">
          {stages.map((stage, idx) => (
            <div key={idx} className="flex items-center gap-2 text-[10px] font-bold text-slate-605 animate-fade-in">
              <Check className="text-blue-500 stroke-[3]" size={12} />
              <span>{stage}</span>
            </div>
          ))}
        </div>
        ...
        <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-blue-500 h-full rounded-full animate-shimmer" style={{ width: '80%' }} />
        </div>
